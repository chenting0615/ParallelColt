...
class DiagonalDComplexMatrix2DTest(arg0: String) extends DComplexMatrix2DTest(arg0) {
  ...
  def testAssignDoubleArray() {
    val expected = Array.ofDim[Double](2 * DLENGTH)
  ...
  def testAssignDoubleArrayArray() {
    val expected = Array.ofDim[Double](NROWS, 2 * NCOLUMNS)
  ...
  def testAssignComplexComplexFunction() {
    ...
    if (DINDEX >= 0) {
      for (r <- 0 until DLENGTH) {
        val expected = DComplex.acos(Acopy.getQuick(r, r + DINDEX))
        assertEquals(expected(0), A.getQuick(r, r + DINDEX)(0), TOL)
        assertEquals(expected(1), A.getQuick(r, r + DINDEX)(1), TOL)
      }
    } else {
      for (r <- 0 until DLENGTH) {
        val expected = DComplex.acos(Acopy.getQuick(r - DINDEX, r))
  ...
  def testAssignComplexMatrix2DComplexComplexComplexFunction() {
    ...
    if (DINDEX >= 0) {
      for (r <- 0 until DLENGTH) {
        assertEquals(DComplex.div(Acopy.getQuick(r, r + DINDEX), B.getQuick(r, r + DINDEX))(0), A.getQuick(r,
          r + DINDEX)(0), TOL)
        assertEquals(DComplex.div(Acopy.getQuick(r, r + DINDEX), B.getQuick(r, r + DINDEX))(1), A.getQuick(r,
          r + DINDEX)(1), TOL)
      }
    } else {
      for (r <- 0 until DLENGTH) {
        assertEquals(DComplex.div(Acopy.getQuick(r - DINDEX, r), B.getQuick(r - DINDEX, r))(0), A.getQuick(r - DINDEX,
          r)(0), TOL)
        assertEquals(DComplex.div(Acopy.getQuick(r - DINDEX, r), B.getQuick(r - DINDEX, r))(1), A.getQuick(r - DINDEX,
          r)(1), TOL)
      }
    }
  }

  def testAssignDComplexMatrix2DDComplexDComplexFunctionIntArrayListIntArrayList() {
  ...
  def testGetNonZeros() {
    A.assign(0, 0)
    val elem1 = Array(0.7, 0.8)
    val elem2 = Array(0.1, 0.2)
    if (DINDEX >= 0) {
    ...
    if (DINDEX >= 0) {
      ...
      val B = A.viewSelection(new DComplexMatrix1DProcedure() {
        ...
        def apply(element: DComplexMatrix1D): Boolean = {
          if (DComplex.abs(DComplex.minus(element.getQuick(NROWS / 4 + DINDEX), value)) <
            TOL) {
            return true
          } else {
            return false
      ...
      val B = A.viewSelection(new DComplexMatrix1DProcedure() {
        ...
        def apply(element: DComplexMatrix1D): Boolean = {
          if (DComplex.abs(DComplex.minus(element.getQuick(NROWS / 4), value)) <
            TOL) {
            return true
          } else {
            return false
  ...
  def testZMultDComplexMatrix2DDComplexMatrix2DDComplexDComplexBooleanBoolean() {
    val alpha = Array(3, 4)
    val beta = Array(5, 6)
    var C = new DiagonalDComplexMatrix2D(NROWS, NROWS, 0)
    for (i <- 0 until DLENGTH) {
      C.setQuick(i, i, Math.random(), Math.random())
    }
    var expected = C.toArray()
    C = A.zMult(Bt, C, alpha, beta, false, false)
    var elem = Array.ofDim[Double](2)
    ...
    C = null
    C = A.zMult(Bt, C, alpha, beta, false, false)
    expected = Array.ofDim[Double](NROWS, 2 * NROWS)
    for (j <- 0 until NROWS; i <- 0 until NROWS) {
      var s = Array.ofDim[Double](2)
      for (k <- 0 until NCOLUMNS) {
        s = DComplex.plus(s, DComplex.mult(A.getQuick(i, k), Bt.getQuick(k, j)))
    ...
    C = null
    C = A.zMult(B, C, alpha, beta, true, false)
    expected = Array.ofDim[Double](NCOLUMNS, 2 * NCOLUMNS)
    for (j <- 0 until NCOLUMNS; i <- 0 until NCOLUMNS) {
      var s = Array.ofDim[Double](2)
      for (k <- 0 until NROWS) {
        s = DComplex.plus(s, DComplex.mult(DComplex.conj(A.getQuick(k, i)), B.getQuick(k, j)))
