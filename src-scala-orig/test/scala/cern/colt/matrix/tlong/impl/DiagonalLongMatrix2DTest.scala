...
class DiagonalLongMatrix2DTest(arg0: String) extends LongMatrix2DTest(arg0) {
  ...
  def testAssignLong() {
    val value = Math.max(1, rand.nextLong() % A.rows())
    A.assign(value)
    if (DINDEX >= 0) {
      for (r <- 0 until DLENGTH) {
  ...
  def testAssignLongArrayArray() {
    val expected = Array.ofDim[Long](NROWS, NCOLUMNS)
  ...
  def testZMultLongMatrix2DLongMatrix2DLongLongBooleanBoolean() {
    val alpha = 3
    val beta = 5
    var C = new DiagonalLongMatrix2D(NROWS, NROWS, 0)
    ...
    C = null
    C = A.zMult(Bt, C, alpha, beta, false, false)
    expected = Array.ofDim[Long](NROWS, NROWS)
    for (j <- 0 until NROWS; i <- 0 until NROWS) {
      var s = 0
      for (k <- 0 until NCOLUMNS) {
        s += A.getQuick(i, k) * Bt.getQuick(k, j)
    ...
    C = null
    C = A.zMult(B, C, alpha, beta, true, false)
    expected = Array.ofDim[Long](NCOLUMNS, NCOLUMNS)
    for (j <- 0 until NCOLUMNS; i <- 0 until NCOLUMNS) {
      var s = 0
      for (k <- 0 until NROWS) {
        s += A.getQuick(k, i) * B.getQuick(k, j)
