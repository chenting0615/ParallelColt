 ...
 */
@SerialVersionUID(1L)
class SelectedDenseDoubleMatrix2D protected (rows: Int,
    columns: Int,
   ...
   * @param offset
   */
  protected def this(elements: Array[Double],
  ...
  def index(row: Int, column: Int): Long = {
    this.offset + rowOffsets(rowZero + row * rowStride) +
   ...
   */
  def like(rows: Int, columns: Int): StrideMatrix2D = new DenseMatrix2D(rows, columns)
  ...
  protected def haveSharedCellsRaw(other: StrideMatrix2D): Boolean = {
    if (other.isInstanceOf[SelectedDenseDoubleMatrix2D]) {
      val otherMatrix = other.asInstanceOf[SelectedDenseDoubleMatrix2D]
      return this.elements == otherMatrix.elements
    } else if (other.isInstanceOf[DenseMatrix2D]) {
      val otherMatrix = other.asInstanceOf[DenseMatrix2D]
      return this.elements == otherMatrix.elements
    }
    false
  }
