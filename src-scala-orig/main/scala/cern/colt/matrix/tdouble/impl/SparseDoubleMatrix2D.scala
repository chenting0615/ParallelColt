 ...
 */
@SerialVersionUID(1L)
class SparseDoubleMatrix2D(rows: Int,
    columns: Int,
    initialCapacity: Int,
    minLoadFactor: Double,
    maxLoadFactor: Double) extends StrideMatrix2D {
  ...
  def this(reader: MatrixVectorReader) {
    ...
    this.elements = if (info.isSymmetric || info.isSkewSymmetric) new OpenLongDoubleHashMap(2 * rowIndexes.length) else new OpenLongDoubleHashMap(rowIndexes.length)
    insert(rowIndexes, columnIndexes, values)
    if (info.isSymmetric) {
  ...
  def assign(function: cern.colt.function.tdouble.Function1): StrideMatrix2D = {
    if (this.isNoView &&
      function.isInstanceOf[cern.jet.math.tdouble.DoubleMult]) {
  ...
  def assign(source: StrideMatrix2D): StrideMatrix2D = {
    if (!(source.isInstanceOf[SparseDoubleMatrix2D])) {
      return super.assign(source)
    }
    val other = source.asInstanceOf[SparseDoubleMatrix2D]
  ...
  def assign(rowIndexes: Array[Int],
      columnIndexes: Array[Int],
      value: Double,
      function: cern.colt.function.tdouble.DoubleDoubleFunction): SparseDoubleMatrix2D = {
   ...
   */
  def getColumnCompressed(sortRowIndexes: Boolean): SparseCCDoubleMatrix2D = {
    val nnz = cardinality()
    ...
    new SparseCCDoubleMatrix2D(rows, columns, rowIndexes, columnIndexes, values, false, false, sortRowIndexes)
  }
   ...
   */
  def getColumnCompressedModified(): SparseCCMDoubleMatrix2D = {
    val A = new SparseCCMDoubleMatrix2D(rows, columns)
    val nnz = cardinality()
    val keys = elements.keys.elements()
    val values = elements.values.elements()
    for (i <- 0 until nnz) {
   ...
   */
  def getRowCompressed(sortColumnIndexes: Boolean): SparseRCDoubleMatrix2D = {
    val nnz = cardinality()
    val keys = elements.keys.elements()
    val values = elements.values.elements()
    val rowIndexes = Array.ofDim[Int](nnz)
    ...
    new SparseRCDoubleMatrix2D(rows, columns, rowIndexes, columnIndexes, values, false, false, sortColumnIndexes)
  }
   ...
   */
  def getRowCompressedModified(): SparseRCMDoubleMatrix2D = {
    val A = new SparseRCMDoubleMatrix2D(rows, columns)
    val nnz = cardinality()
    val keys = elements.keys.elements()
    val values = elements.values.elements()
    for (i <- 0 until nnz) {
  ...
  private def insert(rowIndexes: Array[Int], columnIndexes: Array[Int], value: Double) {
    val size = rowIndexes.length
    for (i <- 0 until size) {
      val row = rowIndexes(i)
      val column = columnIndexes(i)
  ...
  private def insert(rowIndexes: Array[Int], columnIndexes: Array[Int], values: Array[Double]) {
    val size = rowIndexes.length
    for (i <- 0 until size) {
      val value = values(i)
      val row = rowIndexes(i)
  ...
  protected def like1D(size: Int, offset: Int, stride: Int): StrideMatrix1D = {
    new SparseDoubleMatrix1D(size, this.elements, offset, stride)
  }

  protected def viewSelectionLike(rowOffsets: Array[Int], columnOffsets: Array[Int]): StrideMatrix2D = {
    new SelectedSparseDoubleMatrix2D(this.elements, rowOffsets, columnOffsets, 0)
  }
}
