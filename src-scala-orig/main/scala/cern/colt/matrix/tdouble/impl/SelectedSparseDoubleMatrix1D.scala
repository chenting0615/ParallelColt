 ...
 */
@SerialVersionUID(1L)
class SelectedSparseDoubleMatrix1D protected (size: Int,
   ...
   */
  protected def this(elements: AbstractLongDoubleMap, offsets: Array[Int]) {
  ...
  def reshape(slices: Int, rows: Int, columns: Int): DoubleMatrix3D = {
    ...
    for (s <- 0 until slices; c <- 0 until columns; r <- 0 until rows) {
      M.setQuick(s, r, c, getQuick(idx += 1))
    }
    M
  }
