...
import DoubleStatistic._
//remove if not needed
import scala.collection.JavaConversions._

object DoubleStatistic {
   ...
   */
  val EUCLID = new VectorVectorFunction() {

    def apply(a: StrideMatrix1D, b: StrideMatrix1D): Double = {
      Math.sqrt(a.aggregate(b, DoubleFunctions.plus, DoubleFunctions.chain(DoubleFunctions.square, DoubleFunctions.minus)))
    }
  }
   ...
   */
  val BRAY_CURTIS = new VectorVectorFunction() {

    def apply(a: StrideMatrix1D, b: StrideMatrix1D): Double = {
      a.aggregate(b, DoubleFunctions.plus, DoubleFunctions.chain(DoubleFunctions.abs, DoubleFunctions.minus)) /
   ...
   */
  val CANBERRA = new VectorVectorFunction() {

    var fun: DoubleDoubleFunction = new DoubleDoubleFunction() {

      def apply(a: Double, b: Double): Double = Math.abs(a - b) / Math.abs(a + b)
    }

    def apply(a: StrideMatrix1D, b: StrideMatrix1D): Double = {
      a.aggregate(b, DoubleFunctions.plus, fun)
    }
  }
   ...
   */
  val MAXIMUM = new VectorVectorFunction() {

    def apply(a: StrideMatrix1D, b: StrideMatrix1D): Double = {
      a.aggregate(b, DoubleFunctions.max, DoubleFunctions.chain(DoubleFunctions.abs, DoubleFunctions.minus))
    }
  }
   ...
   */
  val MANHATTAN = new VectorVectorFunction() {
  ...
  trait VectorVectorFunction {
     ...
     */
    def apply(x: cern.colt.matrix.tdouble.StrideMatrix1D, y: StrideMatrix1D): Double
  }
   ...
   */
  def aggregate(matrix: StrideMatrix2D, aggr: Array[hep.aida.tdouble.bin.DoubleBinFunction1D], result: StrideMatrix2D): StrideMatrix2D = {
    val bin = new DynamicDoubleBin1D()
    val elements = Array.ofDim[Double](matrix.rows())
    val values = new cern.colt.list.tdouble.DoubleArrayList(elements)
   ...
   */
  def bin(vector: StrideMatrix1D): DynamicDoubleBin1D = {
    val bin = new DynamicDoubleBin1D()
   ...
   *         only).
   */
  def correlation(covariance: StrideMatrix2D): StrideMatrix2D = {
    var i = covariance.columns()
    while (i >= 0) {
      var j = i
      while (j >= 0) {
        val stdDev1 = Math.sqrt(covariance.getQuick(i, i))
        val stdDev2 = Math.sqrt(covariance.getQuick(j, j))
        val cov = covariance.getQuick(i, j)
        val corr = cov / (stdDev1 * stdDev2)
        covariance.setQuick(i, j, corr)
        covariance.setQuick(j, i, corr)
      }
    }
    var i = covariance.columns()
    while (i >= 0) covariance.setQuick(i, i, 1)
    covariance
  }
   ...
   */
  def covariance(matrix: StrideMatrix2D): StrideMatrix2D = {
    val rows = matrix.rows()
    val columns = matrix.columns()
    val covariance = new cern.colt.matrix.tdouble.impl.DenseMatrix2D(columns, columns)
    val sums = Array.ofDim[Double](columns)
    val cols = Array.ofDim[StrideMatrix1D](columns)
    var i = columns
    while (i >= 0) {
    ...
    while (i >= 0) {
      var j = i + 1
      while (j >= 0) {
        val sumOfProducts = cols(i).zDotProduct(cols(j))
        val cov = (sumOfProducts - sums(i) * sums(j) / rows) / rows
   ...
   */
  def cube(x: StrideMatrix1D, y: StrideMatrix1D, weights: StrideMatrix1D): hep.aida.tdouble.DoubleIHistogram2D = {
    if (x.size != y.size || y.size != weights.size) throw new IllegalArgumentException("vectors must have same size")
    val epsilon = 1.0E-5f
    val distinct = new cern.colt.list.tdouble.DoubleArrayList()
    val vals = Array.ofDim[Double](x.size.toInt)
    val sorted = new cern.colt.list.tdouble.DoubleArrayList(vals)
    x.toArray(vals)
    sorted.sort()
    ...
    distinct.trimToSize()
    val xaxis = new hep.aida.tdouble.ref.DoubleVariableAxis(distinct.elements())
    y.toArray(vals)
    sorted.sort()
    cern.jet.stat.tdouble.DoubleDescriptive.frequencies(sorted, distinct, null)
    if (distinct.size > 0) distinct.add(distinct.get(distinct.size - 1) + epsilon)
    distinct.trimToSize()
    val yaxis = new hep.aida.tdouble.ref.DoubleVariableAxis(distinct.elements())
    val histo = new hep.aida.tdouble.ref.DoubleHistogram2D("Cube", xaxis, yaxis)
    histogram(histo, x, y, weights)
  }
   ...
   *             .
   */
  def cube(x: StrideMatrix1D,
      y: StrideMatrix1D,
      z: StrideMatrix1D,
      weights: StrideMatrix1D): hep.aida.tdouble.DoubleIHistogram3D = {
    ...
    if (distinct.size > 0) distinct.add(distinct.get(distinct.size - 1) + epsilon)
    distinct.trimToSize()
    val zaxis = new hep.aida.tdouble.ref.DoubleVariableAxis(distinct.elements())
    val histo = new hep.aida.tdouble.ref.DoubleHistogram3D("Cube", xaxis, yaxis, zaxis)
    histogram(histo, x, y, z, weights)
  }
   ...
   */
  def demo1() {
    val values = Array(Array(1, 2, 3), Array(2, 4, 6), Array(3, 6, 9), Array(4, -8, -10))
    val factory = DoubleFactory2D.dense
    val A = factory.make(values)
    println("\n\nmatrix=" + A)
    println("\ncovar1=" + covariance(A))
  }
   ...
   * Demonstrates usage of this class.
   */
  def demo2(rows: Int, columns: Int, print: Boolean) {
    println("\n\ninitializing...")
    val factory = DoubleFactory2D.dense
    val A = factory.ascending(rows, columns)
    println("benchmarking correlation...")
    val timer = new cern.colt.Timer().start()
    val corr = correlation(covariance(A))
    timer.stop().display()
    if (print) {
   ...
   * Demonstrates usage of this class.
   */
  def demo3(norm: VectorVectorFunction) {
    val values = Array(Array(-0.9611052f, -0.25421095f), Array(0.4308269f, -0.69932648f), Array(-1.2071029f, 0.62030596f), Array(1.5345166f, 0.02135884f), Array(-1.1341542f, 0.20388430f))
    println("\n\ninitializing...")
    val factory = DoubleFactory2D.dense
    val A = factory.make(values).viewDice()
    println("\nA=" + A.viewDice())
   ...
   */
  def distance(matrix: StrideMatrix2D, distanceFunction: VectorVectorFunction): StrideMatrix2D = {
    val columns = matrix.columns()
    val distance = new cern.colt.matrix.tdouble.impl.DenseMatrix2D(columns, columns)
    val cols = Array.ofDim[StrideMatrix1D](columns)
    var i = columns
    while (i >= 0) {
      cols(i) = matrix.viewColumn(i)
    ...
    while (i >= 0) {
      var j = i
      while (j >= 0) {
        val d = distanceFunction.apply(cols(i), cols(j))
   ...
   */
  def histogram(histo: hep.aida.tdouble.DoubleIHistogram1D, vector: StrideMatrix1D): hep.aida.tdouble.DoubleIHistogram1D = {
    var i = vector.size.toInt
    while (i >= 0) {
   ...
   * @return <tt>histo</tt> (for convenience only).
   */
  def histogram(histo: hep.aida.tdouble.DoubleIHistogram1D, matrix: StrideMatrix2D): hep.aida.tdouble.DoubleIHistogram1D = {
    histo.fill_2D(matrix.elements().asInstanceOf[Array[Double]], matrix.rows(), matrix.columns(), matrix.index(0,
      0).toInt, matrix.rowStride(), matrix.columnStride())
    histo
  }
   ...
   * @return <tt>histo</tt> (for convenience only).
   */
  def histogram(histo: Array[Array[hep.aida.tdouble.DoubleIHistogram1D]],
      matrix: StrideMatrix2D,
      m: Int,
      n: Int): Array[Array[hep.aida.tdouble.DoubleIHistogram1D]] = {
    val rows = matrix.rows()
    val columns = matrix.columns()
    if (m >= rows) {
    ...
    val row_size = rows / m
    val col_size = columns / n
    val height = Array.ofDim[Int](m)
    val width = Array.ofDim[Int](n)
    for (r <- 0 until m - 1) {
      height(r) = row_size
    }
    ...
    if ((nthreads > 1) &&
      ...
      for (j <- 0 until nthreads) {
        val firstRow = j * k
        val lastRow = if ((j == nthreads - 1)) m else firstRow + k
        futures(j) = ConcurrencyUtils.submit(new Runnable() {

          def run() {
            var view: StrideMatrix2D = null
            for (r <- firstRow until lastRow; c <- 0 until n) {
              view = matrix.viewPart(r * row_size, c * col_size, height(r), width(c))
              histo(r)(c).fill_2D(view.elements().asInstanceOf[Array[Double]], view.rows(), view.columns(),
                view.index(0, 0).toInt, view.rowStride(), view.columnStride())
            }
          }
        })
      }
   ...
   */
  def histogram(histo: hep.aida.tdouble.DoubleIHistogram2D, x: StrideMatrix1D, y: StrideMatrix1D): hep.aida.tdouble.DoubleIHistogram2D = {
   ...
   *             <tt>x.size() != y.size() || y.size() != weights.size()</tt>.
   */
  def histogram(histo: hep.aida.tdouble.DoubleIHistogram2D,
      x: StrideMatrix1D,
      y: StrideMatrix1D,
      weights: StrideMatrix1D): hep.aida.tdouble.DoubleIHistogram2D = {
    if (x.size != y.size || y.size != weights.size) throw new IllegalArgumentException("vectors must have same size")
    var i = x.size.toInt
    while (i >= 0) {
   ...
   *             .
   */
  def histogram(histo: hep.aida.tdouble.DoubleIHistogram3D,
      x: StrideMatrix1D,
      y: StrideMatrix1D,
      z: StrideMatrix1D,
      weights: StrideMatrix1D): hep.aida.tdouble.DoubleIHistogram3D = {
   ...
   */
  def viewSample(matrix: StrideMatrix1D, fraction: Double, randomGenerator: DoubleRandomEngine): StrideMatrix1D = {
    val epsilon = 1e-05f
    if (fraction < 0 - epsilon || fraction > 1 + epsilon) throw new IllegalArgumentException()
    if (fraction < 0) fraction = 0
    if (fraction > 1) fraction = 1
    if (randomGenerator == null) randomGenerator = new cern.jet.random.tdouble.engine.DoubleMersenneTwister(System.currentTimeMillis().toInt)
    val ncolumns = Math.round(matrix.size * fraction).toInt
    val max = ncolumns
    val selected = Array.ofDim[Long](max)
    val n = ncolumns
    val N = matrix.size.toInt
    cern.jet.random.tdouble.sampling.DoubleRandomSampler
      .sample(n, N, n, 0, selected, 0, randomGenerator)
    val selectedCols = Array.ofDim[Int](n)
    for (i <- 0 until n) selectedCols(i) = selected(i).toInt
    matrix.viewSelection(selectedCols)
  }
   ...
   * @see cern.jet.random.tdouble.sampling.DoubleRandomSampler
   */
  def viewSample(matrix: StrideMatrix2D,
      rowFraction: Double,
      columnFraction: Double,
      randomGenerator: DoubleRandomEngine): StrideMatrix2D = {
    val epsilon = 1e-05f
    ...
    if (randomGenerator == null) randomGenerator = new cern.jet.random.tdouble.engine.DoubleMersenneTwister(System.currentTimeMillis().toInt)
    val nrows = Math.round(matrix.rows() * rowFraction).toInt
    val ncolumns = Math.round(matrix.columns() * columnFraction).toInt
    val max = Math.max(nrows, ncolumns)
    val selected = Array.ofDim[Long](max)
    var n = nrows
    var N = matrix.rows()
    cern.jet.random.tdouble.sampling.DoubleRandomSampler
      .sample(n, N, n, 0, selected, 0, randomGenerator)
    val selectedRows = Array.ofDim[Int](n)
    for (i <- 0 until n) selectedRows(i) = selected(i).toInt
    n = ncolumns
    N = matrix.columns()
    cern.jet.random.tdouble.sampling.DoubleRandomSampler
      .sample(n, N, n, 0, selected, 0, randomGenerator)
    val selectedCols = Array.ofDim[Int](n)
    for (i <- 0 until n) selectedCols(i) = selected(i).toInt
    matrix.viewSelection(selectedRows, selectedCols)
  }
   ...
   * @see cern.jet.random.tdouble.sampling.DoubleRandomSampler
   */
  def viewSample(matrix: DoubleMatrix3D,
      sliceFraction: Double,
      rowFraction: Double,
      columnFraction: Double,
      randomGenerator: DoubleRandomEngine): DoubleMatrix3D = {
    val epsilon = 1e-05f
    ...
    if (columnFraction > 1) columnFraction = 1
    if (randomGenerator == null) randomGenerator = new cern.jet.random.tdouble.engine.DoubleMersenneTwister(System.currentTimeMillis().toInt)
    val nslices = Math.round(matrix.slices() * sliceFraction).toInt
    val nrows = Math.round(matrix.rows() * rowFraction).toInt
    val ncolumns = Math.round(matrix.columns() * columnFraction).toInt
    val max = Math.max(nslices, Math.max(nrows, ncolumns))
    val selected = Array.ofDim[Long](max)
    var n = nslices
    var N = matrix.slices()
    cern.jet.random.tdouble.sampling.DoubleRandomSampler
      .sample(n, N, n, 0, selected, 0, randomGenerator)
    val selectedSlices = Array.ofDim[Int](n)
    for (i <- 0 until n) selectedSlices(i) = selected(i).toInt
    n = nrows
    N = matrix.rows()
    cern.jet.random.tdouble.sampling.DoubleRandomSampler
      .sample(n, N, n, 0, selected, 0, randomGenerator)
    ...
    val selectedCols = Array.ofDim[Int](n)
    for (i <- 0 until n) selectedCols(i) = selected(i).toInt
    matrix.viewSelection(selectedSlices, selectedRows, selectedCols)
  }
}
 ...
 */
class DoubleStatistic protected () extends AnyRef
