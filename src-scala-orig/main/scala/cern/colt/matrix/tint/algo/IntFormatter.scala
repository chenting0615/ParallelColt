 ...
 */
@SerialVersionUID(1L)
class IntFormatter(format: String) extends AbstractFormatter {
   ...
   */
  protected def form(matrix: IntMatrix1D, index: Int, formatter: Formatter): String = formatter.form(matrix.get(index))

  /**
   * Converts a given cell to a String; no alignment considered.
   */
  protected def form(matrix: AbstractMatrix1D, index: Int, formatter: Formatter): String = {
    this.form(matrix.asInstanceOf[IntMatrix1D], index, formatter)
  }

  /**
   * Returns a string representations of all cells; no alignment considered.
   */
  def format(matrix: IntMatrix2D): Array[Array[String]] = {
  ...
  protected def format(matrix: AbstractMatrix2D): Array[Array[String]] = {
    this.format(matrix.asInstanceOf[IntMatrix2D])
  }
   ...
   */
  protected def lead(s: String): Int = {
    if (alignment == DECIMAL) return indexOfDecimalPoint(s)
    super.lead(s)
  }
   ...
   */
  def toSourceCode(matrix: IntMatrix1D): String = {
    val copy = this.clone().asInstanceOf[IntFormatter]
    copy.setPrintShape(false)
    copy.setColumnSeparator(", ")
    val lead = "{"
    val trail = "};"
    lead + copy toString matrix + trail
  }
   ...
   *            the matrix to format.
   */
  def toSourceCode(matrix: IntMatrix2D): String = {
    val copy = this.clone().asInstanceOf[IntFormatter]
    val b3 = blanks(3)
    copy.setPrintShape(false)
    copy.setColumnSeparator(", ")
   ...
   *            the matrix to format.
   */
  def toSourceCode(matrix: IntMatrix3D): String = {
    val copy = this.clone().asInstanceOf[IntFormatter]
    val b3 = blanks(3)
    val b6 = blanks(6)
    copy.setPrintShape(false)
   ...
   */
  def toString(matrix: IntMatrix1D): String = {
   ...
   *            the matrix to convert.
   */
  def toString(matrix: IntMatrix2D): String = super toString matrix
   ...
   *            the matrix to convert.
   */
  def toString(matrix: IntMatrix3D): String = {
    val buf = new StringBuffer()
    val oldPrintShape = this.printShape
    this.printShape = false
    for (slice <- 0 until matrix.slices()) {
   ...
   *            the matrix to convert.
   */
  protected def toString(matrix: AbstractMatrix2D): String = {
    this toString matrix.asInstanceOf[IntMatrix2D]
  }
   ...
   */
  protected def toTitleString(matrix: IntMatrix2D,
   ...
   * @return the matrix converted to a string.
   */
  private def xtoTitleString(matrix: IntMatrix3D,
      sliceNames: Array[String],
      rowNames: Array[String],
      columnNames: Array[String],
      sliceAxisName: String,
      rowAxisName: String,
      columnAxisName: String,
      title: String): String = {
    if (matrix.size == 0) return "Empty matrix"
    val buf = new StringBuffer()
    for (i <- 0 until matrix.slices()) {
