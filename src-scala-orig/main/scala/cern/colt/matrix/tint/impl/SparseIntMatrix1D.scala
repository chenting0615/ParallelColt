 ...
 */
@SerialVersionUID(1L)
class SparseIntMatrix1D(size: Int, 
    initialCapacity: Int, 
    minLoadFactor: Double, 
    maxLoadFactor: Double) extends IntMatrix1D {
  ...
  def reshape(rows: Int, columns: Int): IntMatrix2D = {
    ...
    val M = new SparseIntMatrix2D(rows, columns)
    val idx = 0
  ...
  def reshape(slices: Int, rows: Int, columns: Int): IntMatrix3D = {
    ...
    val M = new SparseIntMatrix3D(slices, rows, columns)
    val idx = 0
