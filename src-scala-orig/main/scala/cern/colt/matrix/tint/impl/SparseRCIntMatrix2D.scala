...
import SparseRCIntMatrix2D._
//remove if not needed
import scala.collection.JavaConversions._

object SparseRCIntMatrix2D {
 ...
 */
@SerialVersionUID(1L)
class SparseRCIntMatrix2D(rows: Int, columns: Int, nzmax: Int) extends WrapperIntMatrix2D(null) {
  ...
  def assign(source: IntMatrix2D): IntMatrix2D = {
    ...
    checkShape(source)
    if (source.isInstanceOf[SparseRCIntMatrix2D]) {
      val other = source.asInstanceOf[SparseRCIntMatrix2D]
    ...
    } else if (source.isInstanceOf[SparseCCIntMatrix2D]) {
      val other = source.asInstanceOf[SparseCCIntMatrix2D].getTranspose
  ...
  def assign(y: IntMatrix2D, function: cern.colt.function.tint.IntIntFunction): IntMatrix2D = {
    checkShape(y)
    if ((y.isInstanceOf[SparseRCIntMatrix2D]) && (function == cern.jet.math.tint.IntFunctions.plus)) {
      val yy = y.asInstanceOf[SparseRCIntMatrix2D]
      ...
      val cnz = Math.max(columnIndexes.length, Math.min(Integer.MAX_VALUE, rowPointers(rows).toInt + rowPointersY(rows).toInt).toInt)
      val columnIndexesC = Array.ofDim[Int](cnz)
      val valuesC = Array.ofDim[Int](cnz)
      val nrow = rows
      val ncol = columns
   ...
   */
  def getColumnCompressed(): SparseCCIntMatrix2D = {
    val tr = getTranspose
    val cc = new SparseCCIntMatrix2D(rows, columns)
   ...
   */
  def getTranspose(): SparseRCIntMatrix2D = {
    ...
    val valuesT = Array.ofDim[Int](nnz)
    for (p <- 0 until nnz) {
      w(columnIndexes(p)) += 1
    }
    ...
    val T = new SparseRCIntMatrix2D(columns, rows)
  ...
  def like(rows: Int, columns: Int): IntMatrix2D = new SparseRCIntMatrix2D(rows, columns)
  ...
  def zMult(y: IntMatrix1D,
    ...
    if (transposeA) {
      if ((!ignore) && (beta != 1.0)) z.assign(cern.jet.math.tint.IntFunctions.mult(beta))
      if ((nthreads > 1) &&
  ...
  def zMult(B: IntMatrix2D,
    ...
    val ignore = (C == null)
    if (C == null) {
      C = if (B.isInstanceOf[SparseRCIntMatrix2D]) new SparseRCIntMatrix2D(rowsA, p, (rowsA * p)) else new DenseIntMatrix2D(rowsA,
        p)
    }
    ...
    if ((B.isInstanceOf[DenseIntMatrix2D]) && (C.isInstanceOf[DenseIntMatrix2D])) {
      var AA: SparseRCIntMatrix2D = null
    ...
    } else if ((B.isInstanceOf[SparseRCIntMatrix2D]) && (C.isInstanceOf[SparseRCIntMatrix2D])) {
      var AA: SparseRCIntMatrix2D = null
      var BB: SparseRCIntMatrix2D = null
      val CC = C.asInstanceOf[SparseRCIntMatrix2D]
      AA = if (transposeA) getTranspose else this
      BB = if (transposeB) B.asInstanceOf[SparseRCIntMatrix2D].getTranspose else B.asInstanceOf[SparseRCIntMatrix2D]
      val rowPointersA = AA.rowPointers
      val columnIndexesA = AA.columnIndexes
      val valuesA = AA.values
