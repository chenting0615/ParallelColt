...
class FormatterFactory {
   ...
   *                if bad format
   */
  def create(format: String): Formatter = {
    new Formatter() {

      def form(value: Double): String = String.format(format, value)

      def form(value: Float): String = String.format(format, value)

      def form(value: Int): String = String.format(format, value)

      def form(value: Long): String = String.format(format, value)

      def form(value: Array[Double]): String = {
        ...
        String.format(format, value(0)) + " + " + String.format(format, value(1))
      }

      def form(value: Array[Float]): String = {
        if (value(0) == 0 && value(1) == 0) {
          return "0"
        }
        if (value(1) == 0) {
