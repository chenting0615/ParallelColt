...
object DoubleRandomEngine {
  ...
  def makeDefault(): DoubleRandomEngine = {
    new DoubleMersenneTwister(System.currentTimeMillis().toInt)
  }
}

@SerialVersionUID(1L)
abstract class DoubleRandomEngine protected () extends cern.colt.PersistentObject with cern.colt.function.tdouble.Function1 with cern.colt.function.tint.IntFunction with cern.colt.function.tlong.LongFunction {
   ...
   * evaluation.
   */
  def apply(dummy: Double): Double = raw()
   ...
   * function evaluation.
   */
  def apply(dummy: Int): Int = nextInt()
   ...
   */
  def apply(dummy: Long): Long = nextLong()
  ...
  def raw(): Double = {
    ...
    (nextVal & 0xFFFFFFFFL) * 2.3283064365386963E-10
  }
}
