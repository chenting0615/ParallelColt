...
object DoubleQuantileCalc {
   ...
   */
  def binomial(n: Long, k: Long): Double = {
    if (k == 0 || k == n) {
      return 1.0
    }
    if (k > n / 2.0) k = n - k
    ...
    var i = k
    while (i > 0) {
      binomial *= N += 1.toDouble / (i -= 1).toDouble
    }
    binomial
  }
   ...
   */
  def ceiling(value: Double): Long = Math.round(Math.ceil(value))
  ...
  protected def known_N_compute_B_and_K_quick(N: Long, epsilon: Double): Array[Long] = {
    ...
    while (b <= maxBuffers) {
      var h = 3
      while (h <= maxHeight && 
        (h - 2) * Math.round(binomial(b + h - 2, h - 1)).toDouble - 
    ...
    while (b <= maxBuffers) {
      ...
      if (h > Integer.MIN_VALUE) {
        ...
        val tmpK = ceiling(N_double / value)
  ...
  protected def known_N_compute_B_and_K_slow(N: Long, 
    ...
    for (b <- 2 until maxBuffers; h <- 3 until maxHeight) {
      ...
      val tmp = ceiling(N_double / binomial)
      if ((b * tmp < memory) && 
  ...
  def test_B_and_K_Calculation(args: Array[String]) {
    ...
    val deltas = Array(0.0, 0.001, 0.0001, 0.00001)
    val epsilons = Array(0.0, 0.1, 0.05, 0.01, 0.005, 0.001, 0.0000001)
      ...
      for (s <- 0 until sizes.length) {
        ...
        for (d <- 0 until deltas.length) {
          ...
          for (e <- 0 until epsilons.length) {
            ...
            val returnSamplingRate = Array.ofDim[Double](1)
      ...
      while (b <= max_b) {
        ...
        while (h <= max_h) {
          ...
          if (alpha_one_OK || alpha_two_OK) {
            ...
            val k = ceiling(Math.max(d / alpha, (h + 1) / (2.0 * epsilon)))
            if (k > 0) {
              val memory = b * k
