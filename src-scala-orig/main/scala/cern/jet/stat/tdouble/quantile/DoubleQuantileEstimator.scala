...
@SerialVersionUID(1L)
abstract class DoubleQuantileEstimator protected () extends cern.colt.PersistentObject with DoubleQuantileFinder {

  protected var bufferSet: DoubleBufferSet = _

  protected var currentBufferToFill: DoubleBuffer = _
  ...
  def addAllOfFromTo(values: DoubleArrayList, from: Int, to: Int) {
    ...
    var bufferSize = k
    var bufferValues: Array[Double] = null
    if (currentBufferToFill != null) {
   ...
   */
  def clone(): AnyRef = {
    val copy = super.clone().asInstanceOf[DoubleQuantileEstimator]
    if (this.bufferSet != null) {
      copy.bufferSet = copy.bufferSet.clone().asInstanceOf[DoubleBufferSet]
      if (this.currentBufferToFill != null) {
   ...
   */
  def contains(element: Double): Boolean = bufferSet.contains(element)
   ...
   */
  def forEach(procedure: cern.colt.function.tdouble.Procedure1): Boolean = this.bufferSet.forEach(procedure)
   ...
   *         <tt>0.0 &lt;= p &lt;=1.0)</tt>.
   */
  def phi(element: Double): Double = bufferSet.phi(element)

  /**
   * Not yet commented.
   */
  protected def postCollapse(toCollapse: Array[DoubleBuffer]): Unit

  /**
   * Default implementation does nothing.
   */
  protected def preProcessPhis(phis: DoubleArrayList): DoubleArrayList = phis
  ...
  def quantileElements(phis: DoubleArrayList): DoubleArrayList = {
    ...
    val quantileElements = Array.ofDim[Double](phis.size)
    new DoubleArrayList(bufferSet.getValuesAtPositions(fullBuffers, triggerPositions))
  }
