 ...
 */
@SerialVersionUID(1L)
class KnownDoubleQuantileEstimator(b: Int, 
    k: Int, 
    protected var N: Long, 
    protected var samplingRate: Double, 
  ...
  protected var samplingAssistant: DoubleRandomSamplingAssistant = if (this.samplingRate <= 1.0) null else new DoubleRandomSamplingAssistant(DoubleArithmetic.floor(N / samplingRate), 
    N, generator)
   ...
   */
  protected def addInfinities(missingInfinities: Int, buffer: DoubleBuffer) {
    val oldAssistant = this.samplingAssistant
    this.samplingAssistant = null
    var even = true
    for (i <- 0 until missingInfinities) {
      if (even) buffer.values.add(Double.MAX_VALUE) else buffer.values.add(-Double.MAX_VALUE)
      even = !even
    }
  ...
  def clear() {
    ...
    if (assist != null) {
      this.samplingAssistant = new DoubleRandomSamplingAssistant(DoubleArithmetic.floor(N / samplingRate), 
   ...
   */
  def clone(): AnyRef = {
    val copy = super.clone().asInstanceOf[KnownDoubleQuantileEstimator]
    if (this.samplingAssistant != null) copy.samplingAssistant = copy.samplingAssistant.clone().asInstanceOf[DoubleRandomSamplingAssistant]
    copy
  }
  ...
  /**
   */
  protected def preProcessPhis(phis: DoubleArrayList): DoubleArrayList = {
    if (beta > 1.0) {
      phis = phis.copy()
      var i = phis.size
      while (i >= 0) {
  ...
  def quantileElements(phis: DoubleArrayList): DoubleArrayList = {
    ...
    if (partial != null) {
      ...
      this.beta = (this.totalElementsFilled + missingValues) / this.totalElementsFilled.toDouble
    } else {
      this.beta = 1.0
    }
   ...
   */
  protected def removeInfinitiesFrom(infinities: Int, buffer: DoubleBuffer) {
    var plusInf = 0
    var minusInf = 0
    var even = true
