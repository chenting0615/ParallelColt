...
import DoubleEquiDepthHistogram._
//remove if not needed
import scala.collection.JavaConversions._

object DoubleEquiDepthHistogram {

  /**
   * Not yet commented.
   */
  def test(element: Double) {
    val quantileElements = Array(50.0, 100.0, 200.0, 300.0, 1400.0, 1500.0, 1600.0, 1700.0, 1800.0, 1900.0, 2000.0)
    val histo = new DoubleEquiDepthHistogram(quantileElements)
 ...
 */
@SerialVersionUID(1L)
class DoubleEquiDepthHistogram(quantileElements: Array[Double]) extends cern.colt.PersistentObject {

  protected var binBoundaries: Array[Double] = quantileElements
   ...
   */
  def binOfElement(element: Double): Int = {
   ...
   */
  def endOfBin(binIndex: Int): Double = binBoundaries(binIndex + 1)
   ...
   */
  def percentFromTo(from: Double, to: Double): Double = phi(to) - phi(from)
   ...
   *             if <tt>binIndex &lt; 0 || binIndex &gt;= bins()</tt>.
   */
  def startOfBin(binIndex: Int): Double = binBoundaries(binIndex)
}
