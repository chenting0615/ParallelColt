...
object DoubleQuantileFinderFactory {
   ...
   *            get exact result, set <tt>epsilon=0.0</tt>;
   */
  protected def known_N_compute_B_and_K_quick(N: Long, epsilon: Double): Array[Long] = {
    val maxBuffers = 50
    val maxHeight = 50
    val N_double = N
    val c = N_double * epsilon * 2.0
  ...
  protected def known_N_compute_B_and_K_slow(N: Long, 
    ...
    var ret_b = 1
    var ret_k = N
    var sampling_rate = 1.0
    var memory = N
    val logarithm = Math.log(2.0 * quantiles / delta)
    val c = 2.0 * epsilon * N_double
    for (b <- 2 until maxBuffers; h <- 3 until maxHeight) {
      ...
      if (delta > 0.0) {
        ...
        val w = logarithm / (2.0 * epsilon * epsilon)
        val x = 0.5 + 0.5 * Math.sqrt(1.0 + 4.0 * t / u)
  ...
  def newDoubleQuantileFinder(known_N: Boolean, 
    ...
    val finder: KnownDoubleQuantileEstimator = null
    if (known_N) {
      val samplingRate = Array.ofDim[Double](1)
      ...
      new KnownDoubleQuantileEstimator(b.toInt, k.toInt, N, samplingRate(0), generator)
    } else {
      ...
      var preComputeEpsilon = -1.0
      if (resultUnknown(3) == 1) preComputeEpsilon = epsilon
      if (true) {
        if (b1 == 1) return new ExactDoubleQuantileFinder()
        return new UnknownDoubleQuantileEstimator(b1.toInt, k1.toInt, h1.toInt, preComputeEpsilon, generator)
      }
      val samplingRate = Array.ofDim[Double](1)
      ...
      if (b2 * k2 < b1 * k1) {
        ...
        return new KnownDoubleQuantileEstimator(b2.toInt, k2.toInt, N, samplingRate(0), generator)
      }
      if (b1 == 1) return new ExactDoubleQuantileFinder()
      new UnknownDoubleQuantileEstimator(b1.toInt, k1.toInt, h1.toInt, preComputeEpsilon, generator)
    }
  }
   ...
   */
  def newEquiDepthPhis(quantiles: Int): cern.colt.list.tdouble.DoubleArrayList = {
    val phis = new cern.colt.list.tdouble.DoubleArrayList(quantiles - 1)
    var i = 1
    while (i <= quantiles - 1) {phis.add(i / quantiles.toDouble)i += 1
    }
    phis
  }
   ...
   */
  def unknown_N_compute_B_and_K(epsilon: Double, delta: Double, quantiles: Int): Array[Long] = {
   ...
   *         <tt>long[3]==1</tt> if precomputing is better, otherwise 0;
   */
  protected def unknown_N_compute_B_and_K_raw(epsilon: Double, delta: Double, quantiles: Int): Array[Long] = {
    if (epsilon <= 0.0) {
    ...
    val pow = Math.pow(2.0, max_H)
    val logDelta = Math.log(2.0 / (delta / quantiles)) / (2.0 * epsilon * epsilon)
    while (best_b == Long.MAX_VALUE && max_Iterations -= 1 > 0) {
      var b = 2
      while (b <= max_b) {
        var h = 2
        while (h <= max_h) {
          val Ld = DoubleArithmetic.binomial(b + h - 2, h - 1)
          val Ls = DoubleArithmetic.binomial(b + h - 3, h - 1)
          val c = logDelta / Math.min(Ld, 8.0 * Ls / 3.0)
          val beta = Ld / Ls
          val cc = (beta - 2.0) * (max_H - 2.0) / (beta + pow - 2.0)
          val d = (h + 3 + cc) / (2.0 * epsilon)
          val f = c * c + 4.0 * c * d
          if (f < 0.0) //continue
          val root = Math.sqrt(f)
          val alpha_one = (c + 2.0 * d + root) / (2.0 * d)
          val alpha_two = (c + 2.0 * d - root) / (2.0 * d)
 ...
 */
class DoubleQuantileFinderFactory protected () extends AnyRef
