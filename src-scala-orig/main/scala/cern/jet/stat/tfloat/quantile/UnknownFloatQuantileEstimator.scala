...
import UnknownFloatQuantileEstimator._
//remove if not needed
import scala.collection.JavaConversions._

object UnknownFloatQuantileEstimator {

  /**
   * To do. This could faster be done without sorting (min and second min).
   */
  protected def sortAscendingByLevel(fullBuffers: Array[FloatBuffer]) {
    new ObjectArrayList(fullBuffers).quickSortFromTo(0, fullBuffers.length - 1, new java.util.Comparator() {
      ...
      def compare(o1: AnyRef, o2: AnyRef): Int = {
        val l1 = o1.asInstanceOf[FloatBuffer].level()
        val l2 = o2.asInstanceOf[FloatBuffer].level()
 ...
 */
@SerialVersionUID(1L)
class UnknownFloatQuantileEstimator(b: Int, 
    k: Int, 
    h: Int, 
    protected var precomputeEpsilon: Float, 
  ...
  protected var sampler: WeightedFloatRandomSampler = new WeightedFloatRandomSampler(1, generator)
   ...
   */
  def clone(): AnyRef = {
    val copy = super.clone().asInstanceOf[UnknownFloatQuantileEstimator]
    if (this.sampler != null) copy.sampler = copy.sampler.clone().asInstanceOf[WeightedFloatRandomSampler]
    copy
  }
