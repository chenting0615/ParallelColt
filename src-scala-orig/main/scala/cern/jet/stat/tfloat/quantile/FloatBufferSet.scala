 ...
 */
@SerialVersionUID(1L)
class FloatBufferSet(b: Int, k: Int) extends BufferSet {

  protected var buffers: Array[FloatBuffer] = new Array[FloatBuffer](b)
   ...
   * allocated.
   */
  def _getFirstEmptyBuffer(): FloatBuffer = {
    var emptyBuffer: FloatBuffer = null
   ...
   */
  def _getFullOrPartialBuffers(): Array[FloatBuffer] = {
    var count = 0
    var i = buffers.length
    while (i >= 0) {
      if (!buffers(i).isEmpty) count += 1
    }
    val collectedBuffers = Array.ofDim[FloatBuffer](count)
    val j = 0
    var i = buffers.length
    while (i >= 0) {
      if (!buffers(i).isEmpty) {
   ...
   */
  def _getFullOrPartialBuffersWithLevel(level: Int): Array[FloatBuffer] = {
    var count = 0
    var i = buffers.length
    while (i >= 0) {
   ...
   */
  def _getPartialBuffer(): FloatBuffer = {
    var i = buffers.length
    while (i >= 0) {
  ...
  protected def clear(k: Int) {
    var i = b()
    while (i >= 0) this.buffers(i) = new FloatBuffer(k)
   ...
   */
  def clone(): AnyRef = {
    val copy = super.clone().asInstanceOf[FloatBufferSet]
    copy.buffers = copy.buffers.clone()
    var i = buffers.length
    while (i >= 0) {
      copy.buffers(i) = copy.buffers(i).clone().asInstanceOf[FloatBuffer]
    }
    copy
  }
   ...
   *            partially full)
   */
  def collapse(buffers: Array[FloatBuffer]): FloatBuffer = {
    var W = 0
   ...
   */
  protected def getValuesAtPositions(buffers: Array[FloatBuffer], triggerPositions: Array[Long]): Array[Float] = {
    var i = buffers.length
    while (i >= 0) {
      buffers(i).sort()
    }
    val bufferSizes = Array.ofDim[Int](buffers.length)
    val bufferValues = Array.ofDim[FloatFloat,](buffers.length)
    ...
    val outputValues = Array.ofDim[Float](triggerPositionsLength)
  ...
  def phi(element: Float): Float = {
    var elementsLessThanOrEqualToElement = 0.0f
    var i = buffers.length
    while (i >= 0) {
      if (!buffers(i).isEmpty) {
