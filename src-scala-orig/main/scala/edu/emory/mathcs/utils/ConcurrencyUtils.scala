...
import edu.emory.mathcs.utils.ConcurrencyUtils._
...
object ConcurrencyUtils {
   ...
   */
  def waitForCompletion(futures: Array[Future[_]], aggr: DoubleDoubleFunction): Double = {
    val size = futures.length
   ...
   * @return the result of aggregation
   */
  def waitForCompletion(futures: Array[Future[_]], aggr: IntIntFunction): Int = {
    val size = futures.length
   ...
   * @return the result of aggregation
   */
  def waitForCompletion(futures: Array[Future[_]], aggr: LongLongFunction): Long = {
    val size = futures.length
   ...
   * @return the result of aggregation
   */
  def waitForCompletion(futures: Array[Future[_]], aggr: ObjectObjectFunction): AnyRef = {
    val size = futures.length
   ...
   * @return the result of aggregation
   */
  def waitForCompletion(futures: Array[Future[_]], aggr: DComplexDComplexDComplexFunction): Array[Double] = {
    val size = futures.length
    val results = Array.ofDim[Double](size, 2)
   ...
   * @return the result of aggregation
   */
  def waitForCompletion(futures: Array[Future[_]], aggr: FComplexFComplexFComplexFunction): Array[Float] = {
    val size = futures.length
    val results = Array.ofDim[Float](size, 2)
    var a: Array[Float] = null
    try {
      for (j <- 0 until size) {
        results(j) = futures(j).get.asInstanceOf[Array[Float]]
      }
      a = results(0)
      for (j <- 1 until size) {
        a = aggr.apply(a, results(j))
   ...
   * @return the result of aggregation
   */
  def waitForCompletion(futures: Array[Future[_]], aggr: FloatFloatFunction): Float = {
    val size = futures.length
