 ...
 */
@SerialVersionUID(1L)
class MightyStaticDoubleBin1D(protected var hasSumOfLogarithms: Boolean, protected var hasSumOfInversions: Boolean, maxOrderForSumOfPowers: Int)
    extends StaticDoubleBin1D {

  protected var sumOfLogarithms: Double = 0.0

  protected var sumOfInversions: Double = 0.0

  protected var sumOfPowers: Array[Double] = null
   ...
   */
  def harmonicMean(): Double = {
    synchronized {
      DoubleDescriptive.harmonicMean(size, sumOfInversions())
    }
  }
  ...
  protected def setMaxOrderForSumOfPowers(max_k: Int) {
    this.sumOfPowers = if (max_k <= 2) null else Array.ofDim[Double](max_k - 2)
  }
