 ...
 */
@SerialVersionUID(1L)
abstract class AbstractDoubleBin1D protected () extends AbstractDoubleBin with cern.colt.buffer.tdouble.DoubleBufferConsumer {
   ...
   */
  def addAllOf(list: DoubleArrayList) {
    synchronized {
   ...
   */
  def buffered(capacity: Int): cern.colt.buffer.tdouble.DoubleBuffer = {
    synchronized {
      new cern.colt.buffer.tdouble.DoubleBuffer(this, capacity)
    }
  }
  ...
  override def equals(`object`: Any): Boolean = {
    if (!(`object`.isInstanceOf[AbstractDoubleBin1D])) return false
    val other = `object`.asInstanceOf[AbstractDoubleBin1D]
