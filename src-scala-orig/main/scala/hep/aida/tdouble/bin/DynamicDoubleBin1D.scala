...
import DynamicDoubleBin1D._
//remove if not needed
import scala.collection.JavaConversions._

object DynamicDoubleBin1D {

  private def includes(array1: Array[Double],
      array2: Array[Double],
      first1: Int,
      last1: Int,
      first2: Int,
 ...
 */
@SerialVersionUID(1L)
class DynamicDoubleBin1D extends QuantileDoubleBin1D {

  /**
   * The elements contained in this bin.
   */
  protected var elements: DoubleArrayList = new DoubleArrayList()

  /**
   * The elements contained in this bin, sorted ascending.
   */
  protected var sortedElements: DoubleArrayList = new DoubleArrayList(0)
  ...
  def clone(): AnyRef = {
    synchronized {
      val clone = super.clone().asInstanceOf[DynamicDoubleBin1D]
   ...
   */
  def correlation(other: DynamicDoubleBin1D): Double = {
    synchronized {
      synchronized (other) {
   ...
   *             if <tt>size() != other.size()</tt>.
   */
  def covariance(other: DynamicDoubleBin1D): Double = {
    synchronized {
      synchronized (other) {
   ...
   */
  protected def elements_unsafe(): DoubleArrayList = synchronized {
  this.elements
}
  ...
  override def equals(`object`: Any): Boolean = {
    synchronized {
      if (!(`object`.isInstanceOf[DynamicDoubleBin1D])) return false
      if (super != `object`) return false
      val other = `object`.asInstanceOf[DynamicDoubleBin1D]
   ...
   */
  def frequencies(distinctElements: DoubleArrayList, frequencies: IntArrayList) {
    synchronized {
   ...
   */
  private def frequencyMap(): cern.colt.map.tdouble.AbstractDoubleIntMap = {
    synchronized {
      val map = new cern.colt.map.tdouble.OpenDoubleIntHashMap()
      var i = size
      while (i >= 0) {
  ...
  def moment(k: Int, c: Double): Double = {
    synchronized {
      DoubleDescriptive.moment(this.elements, k, c)
    }
  }
   ...
   */
  def quantile(phi: Double): Double = {
    synchronized {
      DoubleDescriptive.quantile(sortedElements_unsafe(), phi)
    }
  }
   ...
   */
  def quantileInverse(element: Double): Double = {
    synchronized {
      DoubleDescriptive.quantileInverse(sortedElements_unsafe(), element)
    }
  }
   ...
   */
  def quantiles(percentages: DoubleArrayList): DoubleArrayList = {
    DoubleDescriptive.quantiles(sortedElements_unsafe(), percentages)
  }
   ...
   *         call.
   */
  def removeAllOf(list: DoubleArrayList): Boolean = {
    synchronized {
  ...
  def sample(n: Int,
      ...
      buffer: cern.colt.buffer.tdouble.DoubleBuffer) {
    synchronized {
      ...
      if (!withReplacement) {
        ...
        val sampler = new cern.jet.random.tdouble.sampling.DoubleRandomSamplingAssistant(n, size, randomGenerator)
        var i = n
        while (i >= 0) {
          if (sampler.sampleNextElement()) buffer.add(this.elements.getQuick(i))
        }
      } else {
        val uniform = new cern.jet.random.tdouble.DoubleUniform(randomGenerator)
        val s = size
        var i = n
        while (i >= 0) {
   ...
   */
  def sampleBootstrap(other: DynamicDoubleBin1D,
      resamples: Int,
      randomGenerator: cern.jet.random.tdouble.engine.DoubleRandomEngine,
      function: DoubleBinBinFunction1D): DynamicDoubleBin1D = {
    synchronized {
      if (randomGenerator == null) randomGenerator = AbstractDoubleDistribution.makeDefaultGenerator()
      val maxCapacity = 1000
      val s1 = size
      val s2 = other.size
      val sample1 = new DynamicDoubleBin1D()
      val buffer1 = sample1.buffered(Math.min(maxCapacity, s1))
      val sample2 = new DynamicDoubleBin1D()
      val buffer2 = sample2.buffered(Math.min(maxCapacity, s2))
      val bootstrap = new DynamicDoubleBin1D()
   ...
   */
  def sortedElements(): DoubleArrayList = {
    synchronized {
   ...
   */
  protected def sortedElements_unsafe(): DoubleArrayList = {
    synchronized {
      sort()
   ...
   */
  def standardize(mean: Double, standardDeviation: Double) {
    synchronized {
  ...
  def sumOfPowers(k: Int): Double = {
    synchronized {
      ...
      DoubleDescriptive.sumOfPowers(this.elements, k)
    }
  }
  ...
  override def toString(): String = {
    synchronized {
      ...
      val distinctElements = new DoubleArrayList()
   ...
   */
  def trimmedMean(s: Int, l: Int): Double = {
    synchronized {
      DoubleDescriptive.trimmedMean(sortedElements_unsafe(), mean(), s, l)
    }
  }
  ...
  protected def updateIncrementalStats() {
    val arguments = Array.ofDim[Double](4)
