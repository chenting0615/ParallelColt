...
class QuantileDoubleBin1D protected () extends MightyStaticDoubleBin1D(false, false, 2) {
  ...
  def clone(): AnyRef = {
    synchronized {
      val clone = super.clone().asInstanceOf[QuantileDoubleBin1D]
      if (this.finder != null) clone.finder = clone.finder.clone().asInstanceOf[DoubleQuantileFinder]
      clone
    }
  }
  ...
  def quantile(phi: Double): Double = {
    synchronized {
      quantiles(new DoubleArrayList(Array(phi))).get(0)
    }
  }
   ...
   *         <tt>0.0 &lt;= phi &lt;=1.0)</tt>.
   */
  def quantileInverse(element: Double): Double = synchronized {
  finder.phi(element)
}
   ...
   */
  def quantiles(phis: cern.colt.list.tdouble.DoubleArrayList): DoubleArrayList = {
    synchronized {
  ...
  def splitApproximately(percentages: DoubleArrayList, k: Int): Array[MightyStaticDoubleBin1D] = {
    synchronized {
      ...
      val subBins = Array.ofDim[Double](1 + k * (percentSize - 1))
      subBins(0) = percent(0)
      var c = 1
      ...
      for (i <- 0 until noOfBins) {
        ...
        var binSumOfPowers: Array[Double] = null
        if (maxOrderForSumOfPowers > 2) {
          binSumOfPowers = Array.ofDim[Double](maxOrderForSumOfPowers - 2)
        }
