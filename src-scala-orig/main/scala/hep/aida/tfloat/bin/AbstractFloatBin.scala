 ...
 */
@SerialVersionUID(1L)
abstract class AbstractFloatBin protected () extends cern.colt.PersistentObject {

  /**
   * Returns <tt>center(0)</tt>.
   */
  def center(): Float = center(0)
   ...
   */
  def center(dimension: Int): Float = synchronized {
  0.5f
}
  ...
  override def equals(otherObj: Any): Boolean = {
    if (!(otherObj.isInstanceOf[AbstractFloatBin])) return false
    val other = otherObj.asInstanceOf[AbstractFloatBin]
   ...
   */
  def error(dimension: Int): Float = synchronized {
  0
}
   ...
   */
  def offset(dimension: Int): Float = 1.0f
   ...
   *            the dimension to be considered.
   */
  def value(dimension: Int): Float = 0
}
