 ...
 */
@SerialVersionUID(1L)
class QuantileFloatBin1D protected () extends MightyStaticFloatBin1D(false, false, 2) {

  protected var finder: FloatQuantileFinder = null
  ...
  def clone(): AnyRef = {
    synchronized {
      val clone = super.clone().asInstanceOf[QuantileFloatBin1D]
      if (this.finder != null) clone.finder = clone.finder.clone().asInstanceOf[FloatQuantileFinder]
      clone
    }
  }
  ...
  def compareWith(other: AbstractFloatBin1D): String = {
    ...
    if (other.isInstanceOf[QuantileFloatBin1D]) {
      val q = other.asInstanceOf[QuantileFloatBin1D]
      buf.append("25%, 50% and 75% Quantiles: " + relError(quantile(0.25f), q.quantile(0.25f)) + 
        ", " + 
        relError(quantile(0.5f), q.quantile(0.5f)) + 
        ", " + 
        relError(quantile(0.75f), q.quantile(0.75f)))
   ...
   * Returns the median.
   */
  def median(): Float = quantile(0.5f)
  ...
  def quantile(phi: Float): Float = {
    synchronized {
      quantiles(new FloatArrayList(Array(phi))).get(0)
    }
  }
   ...
   *         <tt>0.0 &lt;= phi &lt;=1.0)</tt>.
   */
  def quantileInverse(element: Float): Float = synchronized {
  finder.phi(element)
}
   ...
   */
  def quantiles(phis: cern.colt.list.tfloat.FloatArrayList): FloatArrayList = {
    synchronized {
      finder.quantileElements(phis)
    }
  }
   ...
   */
  def sizeOfRange(minElement: Float, maxElement: Float): Int = {
    Math.round(size * 
      (quantileInverse(maxElement) - quantileInverse(minElement)))
  }
   ...
   */
  def splitApproximately(percentages: FloatArrayList, k: Int): Array[MightyStaticFloatBin1D] = {
    synchronized {
      val percentSize = percentages.size
      if (k < 1 || percentSize < 2) throw new IllegalArgumentException()
      val percent = percentages.elements()
      val noOfBins = percentSize - 1
      val subBins = Array.ofDim[Float](1 + k * (percentSize - 1))
      subBins(0) = percent(0)
      var c = 1
      for (i <- 0 until noOfBins) {
        val step = (percent(i + 1) - percent(i)) / k
        var j = 1
        while (j <= k) {
          subBins(c += 1) = percent(i) + j * step
          j += 1
          j += 1
        }
      }
      val quantiles = quantiles(new FloatArrayList(subBins)).elements()
      val splitBins = Array.ofDim[MightyStaticFloatBin1D](noOfBins)
      ...
      for (i <- 0 until noOfBins) {
        val step = (percent(i + 1) - percent(i)) / k
        var binSum = 0
        var binSumOfSquares = 0
        var binSumOfLogarithms = 0
        var binSumOfInversions = 0
        var binSumOfPowers: Array[Float] = null
        if (maxOrderForSumOfPowers > 2) {
          binSumOfPowers = Array.ofDim[Float](maxOrderForSumOfPowers - 2)
        }
        var binMin = quantiles(c += 1)
        val safe_min = binMin
        val subIntervalSize = dataSize * step
        var j = 1
        while (j <= k) {
          val binMax = quantiles(c += 1)
          val binMean = (binMin + binMax) / 2
        ...
        c -= 1
        val binSize = Math.round((percent(i + 1) - percent(i)) * dataSize)
        val binMax = binMin
        binMin = safe_min
   ...
   *            the desired number of subintervals per interval.
   */
  def splitApproximately(axis: hep.aida.tfloat.FloatIAxis, k: Int): Array[MightyStaticFloatBin1D] = {
    synchronized {
      val percentages = new FloatArrayList(new hep.aida.tfloat.ref.FloatConverter().edges(axis))
      percentages.beforeInsert(0, Float.NEGATIVE_INFINITY)
      percentages.add(Float.POSITIVE_INFINITY)
      var i = percentages.size
      while (i >= 0) {
        percentages.set(i, quantileInverse(percentages.get(i)))
      }
      splitApproximately(percentages, k)
    }
  }
  ...
  override def toString(): String = {
    synchronized {
      ...
      buf.append("25%, 50%, 75% Quantiles: " + quantile(0.25f) + ", " + 
        quantile(0.5f) + 
        ", " + 
        quantile(0.75f))
