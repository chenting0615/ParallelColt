...
import cern.colt.matrix.impl.WrapperMatrix2D

class DenseLargeDoubleMatrix2DTest(arg0: String) extends DoubleMatrix2DTest(arg0) {

  protected def createMatrices() {
    A = new DenseLargeDoubleMatrix2D(NROWS, NCOLUMNS)
    B = new MatrixTypes.DenseLargeDoubleMatrix2D(NROWS, NCOLUMNS)
    Bt = new MatrixTypes.DenseLargeDoubleMatrix2D(NCOLUMNS, NROWS)
  }
...
/*
  ...
  def testFft2() {
    val nrows = 64
    val ncolumns = 128
    var A = new DenseLargeDoubleMatrix2D(nrows, ncolumns)
    var Acopy = A.copy()
    A.asInstanceOf[DenseLargeDoubleMatrix2D].fft2()
    A.asInstanceOf[DenseLargeDoubleMatrix2D].ifft2(true)
