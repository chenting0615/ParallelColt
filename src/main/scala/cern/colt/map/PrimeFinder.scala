...
object PrimeFinder {
   ...
   */
  val largestPrime = Integer.MAX_VALUE
  ...
  protected def main(args: Array[String]) {
    val from = Integer.parseInt(args(0))
    val to = Integer.parseInt(args(1))
    statistics(from, to)
  }
   ...
   */
  def nextPrime(desiredCapacity: Int): Int = {
    var i = java.util.Arrays.binarySearch(primeCapacities, desiredCapacity)
    if (i < 0) {
      i = -i - 1
    }
    primeCapacities(i)
   ...
   * Tests correctness.
   */
  protected def statistics(from: Int, to: Int) {
    for (i <- 0 until primeCapacities.length - 1 if primeCapacities(i) >= primeCapacities(i + 1)) throw new RuntimeException("primes are unsorted or contain duplicates; detected at " +
      i +
      "@" +
      primeCapacities(i))
    var accDeviation = 0.0
    var maxDeviation = -1.0
    var i = from
    while (i <= to) {
      val primeCapacity = nextPrime(i)
      val deviation = (primeCapacity - i) / i.toDouble
      if (deviation > maxDeviation) {
        maxDeviation = deviation
        println("new maxdev @" + i + "@dev=" + maxDeviation)
      }
      accDeviation += deviation
      i += 1
    }
    val width = 1 + to.toLong - from
    val meanDeviation = accDeviation / width
    println("Statistics for [" + from + "," + to + "] are as follows")
    println("meanDeviation = " + meanDeviation.toFloat * 100 + " %")
    println("maxDeviation = " + maxDeviation.toFloat * 100 + " %")
  }
}
