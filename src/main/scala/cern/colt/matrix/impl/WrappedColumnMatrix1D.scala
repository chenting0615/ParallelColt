 ...
 */
@SerialVersionUID(1L)
class WrappedColumnMatrix1D[@specialized T: Manifest: Numeric](protected val content2D: Matrix2D[T], column: Int) extends AbstractMatrix1D[T] {
  ...
  override def canSetCellAt(index: Int): Boolean = content2D.canSetCellAt(index, column)

  def getQuick(index: Int): T = {
    content2D.getQuick(index, column)
  }

  def setQuick(index: Int, value: T) {
    content2D.setQuick(index, column, value)
  }
  ...
  override def assignConstant(value: T) = {
    for(rowIdx <- 0 until content2D.columns)
      content2D.setQuick(rowIdx, column, value)
    this
  }

  override def forEachNonZero(f: Function2[Int, T, T]) = {
    content2D.forEachNonZeroInColumn(column, new Function3[Int, Int, T, T]() {
      override def apply(rowIdx: Int, colIdx: Int, value: T): T = f.apply(rowIdx, value)
    })
    this
  }
  ...
  def viewSelection(indexes: Array[Int]): Matrix1D[T] = {
    new WrappedColumnMatrix1D[T](content2D.viewSelection(indexes, null), column)
  }
  ...
  def viewPart(index: Int, width: Int) = {
    new WrappedColumnMatrix1D[T](content2D.viewPart(index, 0, width, content2D.columns), column)
  }
   ...
   */
  def viewFlip() = {
    new WrappedColumnMatrix1D[T](content2D.viewColumnFlip(), column)
  }
}
