 ...
 */
@SerialVersionUID(1L)
class DoubleMersenneTwister(seed: Int) extends DoubleRandomEngine {

  private val N = 624

  private val M = 397

  private val MATRIX_A = 0x9908b0df
  ...
  private val mag0 = 0x0

  private val mag1 = MATRIX_A

  private var mti: Int = 0

  private var mt = new Array[Int](N)
   ...
   */
  def this() {
    this(DoubleMersenneTwister.DEFAULT_SEED)
  }
  ...
  override def clone(): AnyRef = {
    val clone = super.clone().asInstanceOf[DoubleMersenneTwister]
  ...
  protected def nextBlock() {
    ...
    while (kk < N - M) {
      ...
      mt(kk) = mt(kk + M) ^ (y >>> 1) ^ (if ((y & 0x1) == 0) mag0 else mag1)
      kk += 1
    }
    while (kk < N - 1) {
      y = (mt(kk) & UPPER_MASK) | (mt(kk + 1) & LOWER_MASK)
      mt(kk) = mt(kk + (M - N)) ^ (y >>> 1) ^ (if ((y & 0x1) == 0) mag0 else mag1)
      kk += 1
    }
    y = (mt(N - 1) & UPPER_MASK) | (mt(0) & LOWER_MASK)
    mt(N - 1) = mt(M - 1) ^ (y >>> 1) ^ (if ((y & 0x1) == 0) mag0 else mag1)
    this.mti = 0
  }
   ...
   * internal state.
   */
  protected def setSeed(seed: Int) {
