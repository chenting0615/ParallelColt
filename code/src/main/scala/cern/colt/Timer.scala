...
class Timer {
  ...
  private var elapsedTimeVar: Long = 0
  ...
  def minus(other: Timer): Timer = {
    val copy = new Timer()
    copy.elapsedTimeVar = nanos() - other.nanos()
    copy
  }
  ...
  def stop(): Timer = {
    if (baseTime != 0) {
      elapsedTimeVar += (System.nanoTime() - baseTime)
    }
    baseTime = 0
    this
  }
...
/*
  ...
  override def toString: String = {
    "Time=" + String.format("%.4f", this.elapsedTime()) + " secs"
  }
*/
}
