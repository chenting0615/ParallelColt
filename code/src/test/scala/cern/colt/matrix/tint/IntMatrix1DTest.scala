...
abstract class IntMatrix1DTest(arg0: String) extends TestCase(arg0) {

  /**
   * Matrix to test
   */
  protected var A: IntMatrix1D = _

  /**
   * Matrix of the same size as a
   */
  protected var B: IntMatrix1D = _
  ...
  protected def populateMatrices() {
    ...
    for (i <- 0 until A.size.toInt) {
      A.setQuick(i, Math.max(1, rand.nextInt() % A.size).toInt)
    }
    for (i <- 0 until B.size.toInt) {
      B.setQuick(i, Math.max(1, rand.nextInt() % A.size).toInt)
    }
  }
  ...
  def testAggregateIntIntFunctionIntFunctionIntArrayList() {
    ...
    val result = A.aggregate(IntFunctions.plus, IntFunctions.square, indexList)
    assertEquals(expected, result)
  }
  ...
  def testAssignIntMatrix1DIntIntFunction() {
    val Acopy = A.copy()
  ...
  def testGetNegativeValuesIntArrayListIntArrayList() {
    A.assign(0)
    A.setQuick(A.size.toInt / 3, -7)
    A.setQuick(A.size.toInt / 2, -1)
    val indexList = new IntArrayList()
  ...
  def testGetNonZerosIntArrayListIntArrayList() {
    A.assign(0)
    A.setQuick(A.size.toInt / 3, 7)
    A.setQuick(A.size.toInt / 2, 1)
    val indexList = new IntArrayList()
  ...
  def testGetPositiveValuesIntArrayListIntArrayList() {
    A.assign(0)
    A.setQuick(A.size.toInt / 3, 7)
    A.setQuick(A.size.toInt / 2, 1)
    val indexList = new IntArrayList()
  ...
  def testToArrayIntArray() {
    val array = Array.ofDim[Int](A.size.toInt)
    A.toArray(array)
    for (i <- 0 until A.size.toInt) {
  ...
  def testViewSelectionIntProcedure() {
    val b = A.viewSelection(new IntProcedure() {

      def apply(element: Int): Boolean = return element % 2 == 0
    })
  ...
  def testZDotProductIntMatrix1D() {
  ...
  def testZDotProductIntMatrix1DIntInt() {
  ...
  @Test
  def testZDotProductIntMatrix1DIntIntIntArrayList() {
    val indexList = new IntArrayList()
    val valueList = new IntArrayList()
